...
function App() {
  ...
  async function send() { if (!query.trim() || loading) return; dispatch(requestStarted()); try { const response = await api("/agent/query", { method: "POST", body: JSON.stringify({ query: query.trim() }) }); setQuery(""); await refreshTelemetry(); setToast(`Gateway response // ${response.status}`); dispatch(requestFinished()); document.getElementById("audit")?.scrollIntoView({ behavior: "smooth" }); } catch (sendError) { dispatch(requestFailed(sendError.message)); setToast(`Gateway rejected // ${sendError.message}`); } }
