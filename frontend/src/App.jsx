...
import { reportReceived, requestFailed, requestFinished, requestStarted, responseReceived, snapshotReceived, workflowCleared } from "./store";
...
const AGENT_OPTIONS = ["recruiter_agent", "hr_analytics_agent", "senior_hr_agent", "rogue_agent"];
...
function AuditStream() {
  ...
  return <div className="panel"><div className="panel-head"><div className="panel-title">Recent activity</div><div className="panel-tag">{filtered.length} events</div></div>
    ...
    <div className="audit-list">{filtered.map((entry) => { const id = String(entry.id); const kind = statusOf(entry); const open = expanded.has(id); return <article className={`audit-item ${kind} ${open ? "" : "collapsed"}`} key={id}>
...
function Approvals({ toast, onChanged, onResolved }) {
  ...
  async function review(item, approve) {
    ...
    try { const response = await api(`/approvals/${item.id}`, { method: "POST", body: JSON.stringify({ approve, reviewer, confirm_quarantine_override: held && approve }) }); onResolved(response); await onChanged(); toast(approve ? "Request approved" : "Request rejected"); } catch (error) { toast(error.message); }
  }
...
function Guardian({ toast, onChanged }) {
  ...
  return <section className="section" id="guardian" data-reveal><div className="section-head"><div><div className="section-number">04 // Autonomous defense</div><h2 className="section-title">GUARDIAN</h2></div><div><p className="section-note">Independent behavior analysis. Human judgment remains final.</p><button className="outline-button guardian-run" onClick={run}>Run guardian check</button></div></div><div className="guardian-grid">{(guardian.agents || []).map((agent) => <article className={`guardian-card ${agent.status === "quarantined" ? "quarantined" : agent.status === "restricted" ? "restricted" : ""}`} key={agent.agent_id}><span className="guardian-verdict">{agent.status === "quarantined" ? "QUARANTINED" : agent.risk_level || "CLEAR"}</span><div className="guardian-agent">{agent.agent_id} // RISK {agent.risk_score || 0}/10</div><div className="guardian-reason">{agent.reasoning || "No active indicators"}</div>{agent.status === "quarantined" && <button className="outline-button lift-button" onClick={() => lift(agent.agent_id)}>Lift quarantine</button>}</article>)}</div></section>;
}
...
function AgentRisk() {
  ...
  return <section className="section" id="agent-risk" data-reveal><div className="section-head"><div><div className="section-number">03 // Adaptive privilege control</div><h2 className="section-title">AGENT RISK</h2></div><p className="section-note">Runtime state derived from the latest audit window. YAML policy remains unchanged.</p></div><div className="risk-grid">{agents.map((agent) => { const restricted = agent.risk_level === "RESTRICTED"; const quarantined = agent.risk_level === "QUARANTINED"; const latest = latestResponse?.adaptive_risk?.agent_id === agent.agent_id ? latestResponse : null; return <article className={`risk-card ${agent.risk_level?.toLowerCase() || "normal"}`} key={agent.agent_id}><div className="risk-card-head"><div><div className="risk-agent">{agent.agent_id}</div><div className="risk-label">{agent.risk_level || "NORMAL"}</div></div><div className="risk-score">{agent.risk_score || 0}<span>/10</span></div></div>{latest && <div className={`risk-latest ${latest.status === "denied" ? "blocked" : ""}`}>LATEST REQUEST // {latest.status.replaceAll("_", " ")} // RISK {latest.adaptive_risk?.risk_score || agent.risk_score}/10</div>}{restricted && <div className="risk-alert">🚨 BEHAVIORAL RISK DETECTED<br /><span>Agent: {agent.agent_id} // Risk: {agent.risk_score}/10 // Status: RESTRICTED</span></div>}<div className="risk-reason">WHY // {agent.reason || "No behavioral risk indicators observed"}</div><div className="risk-stats"><span>DENIED // {agent.denied_requests || 0}</span><span>SENSITIVE // {agent.sensitive_attempts || 0}</span><span>HIGH-RISK // {agent.high_risk_requests || 0}</span></div><div className="risk-privilege">PRIVILEGE // {quarantined ? "QUARANTINED" : restricted ? "RESTRICTED — risky actions blocked" : agent.privilege_state || "POLICY PRIVILEGES"}</div><button className="outline-button risk-details" onClick={() => setDetails(details === agent.agent_id ? null : agent.agent_id)}>{details === agent.agent_id ? "Hide details" : "Why? / details"}</button>{details === agent.agent_id && <div className="risk-detail"><div>FACTORS // LAST {agent.window_size || 0} EVENTS</div>{(agent.risk_factors || []).map((factor) => <p key={factor}>+ {factor}</p>)}{!(agent.risk_factors || []).length && <p>No violations recorded.</p>}<div className="risk-violations">RECENT VIOLATIONS</div>{(agent.recent_violations || []).map((violation, index) => <p key={`${violation.timestamp}-${index}`}>{violation.decision} // {violation.query}</p>)}</div>}</article>; })}</div></section>;
}
...
function App() {
  ...
  useEffect(() => { setAgentKey(localStorage.getItem(`agentgate_key_${selectedAgent}`) || ""); }, [selectedAgent]);
  async function send() { if (!query.trim() || loading) return; if (agentKey) localStorage.setItem(`agentgate_key_${selectedAgent}`, agentKey); dispatch(workflowCleared()); dispatch(requestStarted()); document.getElementById("governance-path")?.scrollIntoView({ behavior: "smooth", block: "center" }); try { const response = await api("/agent/query", { method: "POST", apiKey: agentKey, body: JSON.stringify({ agent_id: selectedAgent, query: query.trim() }) }); dispatch(responseReceived(response)); setQuery(""); await refreshTelemetry(); setToast(`Gateway response // ${response.status}`); dispatch(requestFinished()); document.getElementById("audit")?.scrollIntoView({ behavior: "smooth" }); } catch (sendError) { dispatch(responseReceived({ status: "authentication_failed", trace: [{ step: "query_received", label: "Query Received", status: "success" }, { step: "authentication", label: "Authentication", status: "failed", error: sendError.message }, { step: "database", label: "Database", status: "blocked" }, { step: "audit_log", label: "Audit Log", status: "not_written" }] })); dispatch(requestFailed(sendError.message)); setToast(`Gateway rejected // ${sendError.message}`); } }
  return <><CustomCursor /><ThreeBackdrop /><div className="noise-overlay" /><div className="react-page"><nav className="nav"><Link className="logo" to="/">AGENT<span>GATE</span></Link><div className="nav-links"><a href="#workspace">Control room</a><a href="#guardian">Guardian</a><a href="#compliance">Compliance</a></div><a className="nav-cta" href="#request">Open console</a></nav>
    <main><Hero query={query} setQuery={setQuery} send={send} loading={loading} selectedAgent={selectedAgent} setSelectedAgent={setSelectedAgent} agentKey={agentKey} setAgentKey={setAgentKey} /><div id="governance-path"><Workflow response={latestResponse} /></div><section className="section" id="workspace" data-reveal><div className="section-head"><div><div className="section-number">01 // Control room</div><h2 className="section-title">LIVE SIGNAL</h2></div><p className="section-note">Every request leaves a trace. Every sensitive field has a reason.</p></div><Metrics /></section><div className="marquee"><div className="marquee-track">INTERPRET <span>///</span> EVALUATE <span>///</span> REDACT <span>///</span> AUDIT <span>///</span> APPROVE <span>///</span></div></div>
      <section className="section" id="audit" data-reveal><div className="section-head"><div><div className="section-number">02 // Evidence</div><h2 className="section-title">AUDIT STREAM</h2></div><p className="section-note">Recent events, decisions and redaction evidence.</p></div><div className="workspace-grid"><AuditStream /><Approvals toast={setToast} onChanged={refreshTelemetry} onResolved={(response) => dispatch(responseReceived(response))} /></div></section><AgentRisk /><Guardian toast={setToast} onChanged={refreshTelemetry} /><Compliance toast={setToast} /></main><footer className="footer"><span><strong>AGENTGATE</strong> // governed intelligence</span><span>{connected ? "API LINK // ONLINE" : "API LINK // OFFLINE"} // hr_records // 3400 rows</span></footer></div>{(loading || error || toast) && <div className="toast show">{loading ? "EVALUATING..." : error || toast}</div>}<HeroMotion /></>;
}

function Hero({ query, setQuery, send, loading, selectedAgent, setSelectedAgent, agentKey, setAgentKey }) { return <><section id="hero"><div className="hero-grid" /><div className="hero-orb" /><div className="hero-meta">// Autonomous governance<br />// hr_records / 3,400 records<br />// system online</div><div className="hero-kicker">Your agents can ask. They cannot decide.</div><h1 className="hero-title"><span className="hero-line"><span className="hero-word">AGENTS</span></span><span className="hero-line"><span className="hero-word accent">WITH</span></span><span className="hero-line"><span className="hero-word">BOUNDARIES</span></span></h1><div className="hero-bottom"><p className="hero-desc">AgentGate is the control layer between <strong>natural language</strong> and your database. Interpret intent. Enforce policy. Redact risk. Keep proof.</p><div className="scroll-note">Scroll to inspect<br />↓ ↓ ↓</div></div><div className="query-card" id="request"><div className="query-label">// Send governed request</div><div className="agent-picker"><label htmlFor="agent-select">Identity / agent</label><select id="agent-select" value={selectedAgent} onChange={(event) => setSelectedAgent(event.target.value)}>{AGENT_OPTIONS.map((agent) => <option value={agent} key={agent}>{agent}</option>)}</select></div><input className="agent-key" type="password" value={agentKey} onChange={(event) => setAgentKey(event.target.value)} placeholder="Bearer key for selected agent" autoComplete="off" /><textarea value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if ((event.metaKey || event.ctrlKey) && event.key === "Enter") send(); }} rows="3" placeholder="Ask your data something..." /><div className="query-row"><span className="query-hint">Key stays local // ⌘ + ENTER</span><button className="lime-button" onClick={send} disabled={loading}>{loading ? "Evaluating..." : "Transmit ↗"}</button></div></div></section><div className="marquee"><div className="marquee-track">INTERPRET <span>///</span> EVALUATE <span>///</span> REDACT <span>///</span> AUDIT <span>///</span> APPROVE <span>///</span></div></div></>; }
